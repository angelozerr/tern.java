(function(mod) {
  ...
  function postInfer(ast, scope) {
    ...
    var filename = ast.sourceFile.name, server = infer.cx().parent, mods = server.mod.angular.modules;
    if (mods) {      
      for (var name in mods) {
        var mod = mods[name];
    ...
    if (server.sendToClient) {
      var data = {};
      for (var name in mods) {
        var modItem = data[name] = {};
        var mod = mods[name], fields = mod.injector.fields;
        for(var fieldName in fields) {
          modItem[fieldName] = {};
        }
      }
      server.sendToClient("angular:modulesChanged", data);
    }
