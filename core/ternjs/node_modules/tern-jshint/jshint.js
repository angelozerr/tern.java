(function(mod) {
  ...
  function JSHintConfig(server, options) {
    if (options.config) {
      ...
      if (!fs.existsSync(filepath)) {
        ...
        if (!fs.existsSync(filepath)) {
          throw new Error("Cannot find jshint config file " + filepath);
        }
      }
      this.update();
    }
  ...
  JSHintConfig.prototype.update = function() {
    ...
    if (filepath) {
      var fs = require("fs"), current = fs.statSync(filepath).mtime;
      if (!this.mtime || (this.mtime.getTime() != current.getTime())) {
  ...
  function validate(server, query, file, messages) {
	...
	// Update jshint config if needed.
	jshintCfg.update();
	var config = jshintCfg.config, globals = jshintCfg.globals;
