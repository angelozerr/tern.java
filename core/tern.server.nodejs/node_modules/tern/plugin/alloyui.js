(function(mod) {
  ...
  function yuiProto(cx) {
    var ngDefs = cx.definitions.alloyui;
    return ngDefs && ngDefs.YUI.getProp("prototype").getType();
  }

  infer.registerFunction("alloyui_fnInstance", function(self, args, argNodes) {
    var yui = self.getType();
    if (yui && argNodes && argNodes.length > 1) {
      
      var result = new infer.AVal;      
      //fnType.propagate(new infer.IsCallee(infer.cx().topScope, deps, null, result));
      
      var fn = args[1];
      var A = new infer.Obj(yui, fn.argNames[0]); 
      var s = [];
      s.push(A)
      fn.propagate(new infer.IsCallee(infer.ANull, s, null, result));
      //fn.propagate(new infer.IsCallee(infer.cx().topScope, A, null, result));
      //applyWithInjection(yui, args[arg1], argNodes[arg1]);
    }
  });
  
  infer.registerFunction("alloyui_alloyInstance", function(_self, args, argNodes) {
    var cx = infer.cx(), data = cx.parent._AlloyUI;
    if (data.Alloy) return data.Alloy;      
    var proto = yuiProto(cx);
    data.Alloy = proto;
    return proto;
    //var instance = new infer.Obj(proto || true);
    //data.Alloy = instance;
    //return instance;
  });
  
  infer.registerFunction("AUI", function(_self, args, argNodes) {
  ...
  var defs = {
    ...
    "!define": {
      YUI: {
        prototype: {
          use: {
            "!type": "fn(modules: [string], callback: fn(A: ?)) -> !this",
            "!effects": ["custom alloyui_fnInstance"],
            "!url": "http://alloyui.com/versions/1.5.x/api/classes/YUI.html#method_use",
    ...
    AUI: {
      "!type": "fn() -> !custom:alloyui_alloyInstance",
      "!url": "...",
      "!doc": "...",      
    },
    requirejs: {
