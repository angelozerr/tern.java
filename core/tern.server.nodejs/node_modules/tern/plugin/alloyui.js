(function(mod) {
  ...
  function injectModule(A, name, isPrototype) {
    var cx = infer.cx(), ngDefs = cx.definitions.alloyui, module = ngDefs[name];
    if (module) { 
      if (isPrototype) {
        module.getProp('prototype').getType().propagate(A);
      } else {
        module.getType().propagate(A);
      }
    }
  }
    ...
    if (yui && argNodes) {
      ...
      if (lastArg.type == "FunctionExpression") {
        ...
        if (fn.argNames  && fn.argNames.length > 0) {              
          var A = fn.args[0];
          injectModule(A, "YUI", true);          
          for ( var i = 0; i < argNodes.length - 1; i++) {
            var node = argNodes[i];
            if (node.type == "Literal" && typeof node.value == "string") {
              injectModule(A, node.value);
            } else if (node.type == "ArrayExpression") for (var i = 0; i < node.elements.length; ++i) {
              var elt = node.elements[i];
              if (elt.type == "Literal" && typeof elt.value == "string") {
                injectModule(A, elt.value);
              }
            }
          }
        }        
