		...
		case 'module':
			...
			findModules(_angular, files, function(moduleName, module) {
				if (startsWithString(moduleName, expression))
		      	  completions.push({"name" : moduleName, "type" : infer.toString(module)})
			});      	  
			break;
		case 'controller':
			// find controller
			...
			findControllers(_angular, files, moduleName, function(name, node, fnType, scopeArg) {
				if (startsWithString(name, expression))
		      	  completions.push({"name" : name, "type" : infer.toString(fnType)})
			});
			break;
		default:
			findModels(_angular, files, scope, function(forward) {
				var prop = forward.prop;
				if (prop && startsWithString(prop, expression)) 
					completions.push({"name" : prop, "type" : infer.toString(forward.type)});
			});
			break;
      }
      return result;	  
