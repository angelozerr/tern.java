(function(mod) {
  ...
  function findModules(_angular, files, expression, startsWith, c) {
		for ( var moduleName in _angular.modules) {
			var result = startsWith ? startsWithString(moduleName, expression) : moduleName == expression; 
	          if (result) {
  ...
  function getModuleControllers(moduleName, _angular) {
	 var controllers = [];
     ...
     if (module) {
       ...
       for (var fieldName in fields) {
           ...
           if (field.type === "controller") {
        	   var arg = getScopeArg(field.fnType);
        	   if (arg) {
        		   controllers.push({name: fieldName, "$scope" : arg});
        	   }
           }         
       }
     }
  ...
  function getLocalControllers(files) {
	  var controllers = [];
				  ...
				  var item = props[prop];
	    	  		if (item.types && item.types.length > 0) {
	    	  			for(var i=0; i<item.types.length; i++) {
	    	  				...
	    	  				var arg = getScopeArg(type);
			  				if (arg) {
			  					controllers.push({"name" : type.name, "$scope" : arg})    		  					
			  				}
	    	  			}
	    	  		}	  
			  }
  ...
  function findControllers(_angular, files, completions, expression, scope) {
	  var moduleName = scope.module;
	  var controllers = null;
	  if (moduleName) {
		// find controllers of given module
		controllers = getModuleControllers(moduleName, _angular);		
	  } else {
		 // find global controllers of the given file
		 controllers = getLocalControllers(files);		  
	  }
	  for ( var i = 0; i < controllers.length; i++) {
			var controller = controllers[i];
			if (startsWithString(controller.name, expression))
			completions.push({"name" : controller.name, "type" : "controller"});
	  }
  }
  
  function findModels(_angular, files, completions, expression, scope) {
	  var moduleName = scope.module;
	  ...
	  if (moduleName) {
		...
		controllers = getModuleControllers(moduleName, _angular);		
		 ...
		 // find global controllers of the given files
		 controllers = getLocalControllers(files);		  
	  }
	  var controller = null
	  for ( var i = 0; i < controllers.length; i++) {
	  ...
	  if (controller) {
		  if (controller.$scope) {
			  var forward = controller.$scope.forward;
			  if (forward) {
				  for ( var i = 0; i < forward.length; i++) {
					var prop = forward[i].prop;
					if (prop && startsWithString(prop, expression))
						completions.push({"name" : prop, "type" : "model"});
				}
			  }
		  }
	  }
  ...
  function findCompletions(_angular, files, expression, scope, angularType, query) {
      ...
      switch (angularType) {
		case 'module':
			// find modules
			findModules(_angular, files, expression, true, function(moduleName, module) {
		      	  completions.push({"name" : moduleName, "type" : infer.toString(module)})
			});      	  
			break;
		case 'controller':
			// find controller
			findControllers(_angular, files, completions, expression, scope);
			break;
		default:
			findModels(_angular, files, completions, expression, scope);
			break;
      }
      return result;	  
  }
  ...
  function findDef(_angular, files, expression, scope, angularType, query) {	  
	  ...
	  switch (angularType) {
		case 'module':
			// find modules
			findModules(_angular, files, expression, false, function(moduleName, module) {
				type = module;
			});      	  
			break;
		case 'controller':
