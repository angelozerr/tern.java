(function(mod) {
  ...
  tern.defineQueryType("angular", {
    run : function(server, query) {
      ...
      if (angularType == null) throw ternError("missing .query.angularType field");
  ...
  function getScopeController(_angular, files, moduleName, controllerName) {
    var scopeArg = null;
    visitControllers(_angular, files, moduleName, function(name, n, fnType,
        arg) {
  ...
  function visitControllers(_angular, files, moduleName, c) {
    if (moduleName) {
      ...
      visitModuleControllers(_angular, files, moduleName, c);
    } else {
  ...
  function findModels(_angular, files, moduleName, controllerName, c) {
  ...
  function findCompletions(_angular, files, expression, scope, angularType,
      query) {
    var completions = [];
    var result = {
    ...
    function createCompletionIfMatch(prop, obj, module, controller) {
      if (startsWithString(prop, word)) {
        completions.push(createCompletion(prop, obj, scope, query, module, controller));
      }
    }
    ...
    case 'module':
      ...
      visitModules(_angular, files, function(moduleName, module) {
        createCompletionIfMatch(moduleName, module, moduleName)
      });
      break;
    case 'controller':
      // find controller
      ...
      visitControllers(_angular, files, moduleName, function(name, node,
          fnType, scopeArg) {
        createCompletionIfMatch(name, fnType, moduleName, name)
      });
      break;
    default:

      var moduleName = scope.module, controllers = scope.controllers;
      if (controllers) {
        for ( var j = 0; j < controllers.length; j++) {
          var controllerName = controllers[j], scopeCtrl = getScopeController(_angular, files, moduleName,
              controllerName);
          if (context) {
            var root = scopeCtrl;
            ...
            findModels(_angular, files, moduleName, controllerName, function(forward) {
              ...
              if (prop && forward.type) {
                createCompletionIfMatch(prop, forward.type, moduleName, controllerName);
              }
            });
            if (scope.repeat) {
              for ( var prop in scope.repeat) {
                if (startsWithString(prop, word)) {
                  ...
                  completions.push(createCompletion(prop, obj, scope, query, moduleName,
                      controllerName));
                }
              }
            }
  ...
  function createCompletion(name, node, scope, query, module, controller) {
	  ...
	  if (controller) completion.controller = controller;	  
  	  return completion;
  }
  ...
  function findType(_angular, files, expression, scope, angularType, query) {	  
	  ...
	  switch (angularType) {
		...
		case 'controller':
			var moduleName = scope.module;
			visitControllers(_angular, files, moduleName, function(n, node, fnType, scopeArg) {
				if (n == expression) {
					name = n;
					type = fnType;
					return true;
