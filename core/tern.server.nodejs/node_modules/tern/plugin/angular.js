(function(mod) {
  ...
  function findModules(_angular, completions, expression) {
		for ( var moduleName in _angular.modules) {
	          if (startsWithString(moduleName, expression)) {
	        	  completions.push({"name" : moduleName, "type" : "module"})
	          }
	    }
  }
  ...
  var querySubTypes = {
    ...
    run : function(server, query) {
      ...
      return querySubTypes[subtype].run(_angular, files, expression, scope, angularType);
     /* var wordStart = expression.length, wordEnd = wordStart, text = expression;
  ...
  function findCompletions(_angular, files, expression, scope, angularType) {
      ...
      switch (angularType) {
		case 'module':
			// find modules
			findModules(_angular, completions, expression);
			break;
		case 'controller':
			// find controller
