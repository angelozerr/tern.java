(function(mod) {
  ...
  function getModule(_angular, files, moduleName) {
	  var module = null;
	  findModules(_angular, files, function(name, mod) {
		if (moduleName == name ) {
			module = mod;
			return true;
		}
  ...
  function findModules(_angular, files, c) {
		for ( var moduleName in _angular.modules) {
			var module = _angular.modules[moduleName];
	        if (c(moduleName, module)) break;	        
	    }
  }

  function findModuleControllers(_angular, files, moduleName,  c) {
	 var module = getModule(_angular, files, moduleName);
     if (module) {
       var fields = module.injector.fields, stop = false;
       for (var fieldName in fields) {
           ...
           if (field.type === "controller") {
        	   ...
        	   var scopeArg = getScopeArg(fnType);
 				if (scopeArg) {
 					stop = (c(fieldName, field.originNode, fnType, scopeArg));  		  					
 				}
           }         
       }
     }
  ...
  function findGlobalControllers(_angular, files, c) {
	  var stop = false;
	  for ( var j = 0; j < files.length; j++) {
		  if (stop) break;
		  var props = files[j].scope.props;
		  if (props) {
				  ...
				  var item = props[prop];
	    	  		if (item.types && item.types.length > 0) {
	    	  			for(var i=0; i<item.types.length; i++) {
	    	  				...
	    	  				var scopeArg = getScopeArg(fnType);
			  				if (scopeArg) {
			  					stop = (c(fnType.name, fnType.originNode.id, fnType, scopeArg));   		  					
			  				}
	    	  			}
	    	  		}	  
			  }
  ...
  function findControllers(_angular, files, moduleName, c) {
	  if (moduleName) {
		// find controllers of given module
		findModuleControllers(_angular, files, moduleName, c);		
	  } else {
		 // find global controllers of the given file
		  findGlobalControllers(_angular, files, c);			  
	  }
  }
  
  function getScopeController(_angular, files, moduleName, controllerName) {
	  var scopeArg = null;
	  findControllers(_angular, files, moduleName, function(name, n, fnType, arg) {
  ...
  function findModels(_angular, files, scope, c) {
	  var moduleName = scope.module;
	  var controllerName = scope.controller;
	  var scopeArg = getScopeController(_angular, files, moduleName, controllerName);
	  if (scopeArg) {
		  var forward = scopeArg.forward;
		  if (forward) {
  ...
  var querySubTypes = {
    ...
    run : function(server, query) {
      ...
      return querySubTypes[subtype].run(_angular, files, expression, scope, angularType, query);
    }
  });
  ...
  function findCompletions(_angular, files, expression, scope, angularType, query) {
      var completions = [];
      ...
      var word = '', current = '', context = null;
		...
		for ( var i = length-1; i >= 0; i--) {
			if (acorn.isIdentifierChar(expression.charCodeAt(i))) current=expression.charAt(i) + current;		
			else if (expression.charAt(i) === '.') {
				if (context) context.unshift(current); else {word = current; context = [];}
				current = '';
			} else {
				break;
			}
		}
		if (context) context.unshift(current); else word = current; 
		
		function gather(prop, obj, depth) {
			if (startsWithString(prop, word)) {
				completions.push(createCompletion(prop, obj, query));
			}
		}
      ...
      switch (angularType) {
		case 'module':
			// find modules
			findModules(_angular, files, function(moduleName, module) {
				if (startsWithString(moduleName, expression))
		      	  completions.push(createCompletion(moduleName, module, query));
			});      	  
			break;
		case 'controller':
			// find controller
			var moduleName = scope.module;
			findControllers(_angular, files, moduleName, function(name, node, fnType, scopeArg) {
				if (startsWithString(name, expression))
				  completions.push(createCompletion(name, fnType, query));
			});
			break;
		default:
			
			var moduleName = scope.module, controllerName = scope.controller;
			var scopeCtrl = getScopeController(_angular, files, moduleName, controllerName);
			if (context) {
				var root = scopeCtrl;
				for ( var i = 0; i < context.length; i++) {		
					var prop = context[i];
					if (scope.repeat && scope.repeat[prop]) {
						var arrProp = scope.repeat[prop]; // case when ngRepeat;
						var arr = getType(root, arrProp);
						root = arr.type.getProp("<i>").getType();
					} else {
				...
				if (root) {
					//if (root.type) {
						infer.forAllPropertiesOf(root.type ? root.type : root, gather);
					//}					
				}
				
			} else {
				findModels(_angular, files, scope, function(forward) {
					var prop = forward.prop;
					if (prop &&  forward.type) {
						if (startsWithString(prop, word))						
							completions.push(createCompletion(prop, forward.type, query));
					}
				});
				if (scope.repeat) {
					for (var prop in scope.repeat) {
						if (startsWithString(prop, word)) {
							var arrProp = scope.repeat[prop]; // case when ngRepeat;
							var arr = getType(scopeCtrl, arrProp);
							var obj = arr.type.getProp("<i>").getType();
							completions.push(createCompletion(prop, obj, query));
						}
					}
				}
  ...
  function createCompletion(name, node, query) {
  ...
  function findDef(_angular, files, expression, scope, angularType, query) {	  
	  var node = null;
	  switch (angularType) {
		case 'module':
			// find modules
		...
		case 'controller':
			var moduleName = scope.module;
			findControllers(_angular, files, moduleName, function(name, n, fnType, scopeArg) {
				if (name == expression) {
					node = n;
					return true;
				}
  ...
  function findType(_angular, files, expression, scope, angularType, query) {	  
	  ...
	  switch (angularType) {
		...
		case 'controller':
			var moduleName = scope.module;
			findControllers(_angular, files, moduleName, function(n, node, fnType, scopeArg) {
				if (n == expression) {
					name = n;
					type = fnType;
					return true;
	  ...
	  if (type) {
		  return {type: infer.toString(type), name: name, origin: type.origin}
	  }
	  return {};
  }
