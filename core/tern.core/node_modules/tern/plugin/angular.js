(function(mod) {
  ...
  function getExpressionType(root, context, scopeProps, controllerAs) {
    var obj = root;
    for ( var i = 0; i < context.length; i++) {
      var prop = context[i];
      if (i == 0) {
        if (prop === controllerAs) {
          // do nothing
        } else {
          if (scopeProps && scopeProps[prop] && scopeProps[prop].repeat) {
            var arrProp = scopeProps[prop].repeat; // case when ngRepeat;
            var index = arrProp.indexOf('.');
            if (index == -1) {
              obj = getArrType(obj, arrProp);
            } else {
              obj = getExpressionType(root, arrProp.substring(0, index).split("."), null, controllerAs);   
              if (obj) obj = getArrType(obj, arrProp.substring(index + 1, arrProp.length));
            }
          } else {
            obj = getType(obj, prop);
          }
        }
      } else {
        obj = getType(obj, prop);
        ...
        if (controllers) {
          for ( var j = 0; j < controllers.length; j++) {
            ...
            if (scopeCtrl) {
              ...
              if (context) {
                // expression with '.'
                var root  = getExpressionType(scopeCtrl, context, scopeProps, controllerAs);                
                if (root) {
                  if (root === scopeCtrl) {
                    // properties/methods of $scope
                    showMethodsOfScope(scopeCtrl, gather);
                  } else {
  ...
  function showMethodsOfScope(scopeCtrl, gather) {
