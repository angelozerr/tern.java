...
public class TernPluginsBlock extends AbstractTableBlock implements
		ISelectionProvider {
	...
	protected void fillWithWorkspaceProcessors() {
		setProcessors(TernCorePlugin.getTernServerTypeManager()
				.getTernPlugins());
	}
	...
	protected void setProcessors(ITernPlugin[] vms) {
		ternPlugins.clear();
	...
	public ITernPlugin[] getProcessors() {
		return ternPlugins.toArray(new ITernPlugin[ternPlugins.size()]);
	}

	public void setCheckedInstall(List<ITernPlugin> selectedPlugins) {
		if (selectedPlugins == null) {
			setSelection(new StructuredSelection());
		} else {
			setSelection(new StructuredSelection(selectedPlugins));
		}
	}

	public ITernPlugin getCheckedInstall() {
		Object[] objects = tableViewer.getCheckedElements();
		if (objects.length == 0) {
			return null;
		}
		return (ITernPlugin) objects[0];
	}
		...
		public String getColumnText(Object element, int columnIndex) {
			if (element instanceof ITernPlugin) {
				...
				case 1:
					...
					// }
					// return TernUIMessages.InstalledProcessorsBlock_8;
				}
			}
